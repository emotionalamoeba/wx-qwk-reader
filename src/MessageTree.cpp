...
MessageTree::MessageTree(wxWindow *parent, wxWindowID id, const wxPoint &pos,
                         const wxSize &size, long style, const wxString &name) : wxTreeListCtrl(parent, id, pos,
                                                                                                size, style, name)
{
  AppendColumn("#", 70);
...
wxTreeListItem MessageTree::add_message_to_tree(Message *message,
                                                wxTreeListItem parentItem)
{
  ...
  wxTreeListItem item =
      InsertItem(parentItem, wxTLI_FIRST, messageNumberText);
...
void MessageTree::add_children_of_id(
